...
**The two models divide a project along different lines.** A sequential project is divided by activity: a stage is 'requirements' or 'testing', and it ends when that activity is finished for the whole product. **An iterative project is usually divided by functionality**: an iteration is 'the part that does X', and it runs through several activities to deliver it.
