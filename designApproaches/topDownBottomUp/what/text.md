...
**{{ show_term("Multi-level design") }} can be done in a top-down manner, bottom-up manner, or as a mix.**
* {{ show_term("Top-down") }}: Design the high-level design first and flesh out the lower levels later. This is especially useful when designing big and novel systems where the high-level design needs to be stable before lower-level designs can be created.
* {{ show_term("Bottom-up") }}: Design lower-level components first and put them together to create the higher-level system later. This is not usually scalable for bigger systems. One instance where this approach might work is when designing a variation of an existing system or repurposing existing components to build a new system.
