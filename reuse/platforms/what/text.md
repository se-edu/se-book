...
**A {{ show_term("platform") }} provides a runtime environment for applications.** A platform is often bundled with various libraries, tools, frameworks, and technologies in addition to a runtime environment, but the defining characteristic of a software platform is the presence of a runtime environment.<br>
{{ label_example }} %%Technically, an operating system can be called a platform. A Windows PC is a platform for desktop applications, while iOS is a platform for mobile applications.%%
