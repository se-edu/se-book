...
{% set junit_ug = "https://docs.junit.org/5.14.2/_exports/junit-user-guide-5.14.2.html" %}

Given below are some noteworthy JUnit concepts, as per the [JUnit 5 User Guide](https://docs.junit.org/5.14.2/overview.html).
...
* **Conditional test execution:** It is possible to configure tests to run only under certain conditions. For example, `@TestOnMac` annotation can be used to specify tests that should run on Mac OS only. [[more ...]({{ junit_ug }}#writing-tests-conditional-test-execution)]
...
* **Test hierarchies**: Normally, we organize tests into separate test classes. If a more hierarchical structure is needed, the `@Nested` annotation can be used to express the relationship among groups of tests. [[more ...]({{ junit_ug }}#writing-tests-nested-tests)]
...
* **Parameterized tests** make it possible to run a test multiple times with different arguments. The parameter values can be supplied using a variety of ways e.g., an array of values, enums, a csv file, etc. [[more ...]({{ junit_ug }}#writing-tests-parameterized-classes-and-tests)]
