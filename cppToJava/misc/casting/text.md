...
Note that the `dc.catchMice()` line will not compile if `a` is not downcast to a `DomesticCat` object first. Reason: the `catchMice` method is specific to the `DomesticCat` class and not present in the `Animal` or the `Cat` classes.<br>
