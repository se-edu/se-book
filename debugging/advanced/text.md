...
<span id="outcomes">{{ icon_outcome }} Can recognize situations where the basic debugging techniques fall short, and knows what to reach for instead</span>
...
* **Some failures occur only in production**, under real data volumes, real configuration, and real users. With no way to attach a debugger, logging, telemetry, and post-mortem artifacts such as crash and thread dumps are the toolkit.
