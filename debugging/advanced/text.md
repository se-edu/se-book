...
**{{ show_term("Tracing") }} records the execution automatically** — every line executed, or every change to a chosen variable — for examination afterwards. Use it when the failure is too fast to watch, or happens in a run you cannot monitor interactively. It comes at the cost of a large volume of recorded output, and of slowing the program enough to disturb the timing-sensitive failures described below.
...
* **Some failures occur only in production**, under real data volumes, real configuration, and real users. When you cannot attach a debugger, the toolkit is logging, telemetry %%measurements the running system reports about itself%%, and post-mortem artifacts such as crash and thread dumps.
