...
That deployment view in the example looks trivial, which is exactly the point. Right now all the interesting structure is logical. When several users need to share invoices, a server is added (see the client-server style), and then _both_ views change — the deployment view dramatically, the logical view in a smaller but real way.
